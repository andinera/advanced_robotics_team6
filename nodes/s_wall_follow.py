...
def s_heuristic(ir_bottom_pid, ir_top_pid, imu_wall_pid, imu_corner_pid):
    ir_bottom_diff = math.fabs(ir_bottom_pid.reported_states[-1] - ir_bottom_pid.reported_states[-2]) 
    ir_top_diff = math.fabs(ir_top_pid.reported_states[-1] - ir_top_pid.reported_states[-2])
    ir_bottom_err = math.fabs(ir_bottom_pid.reported_states[-1] - ir_bottom_pid.setpoint.data)
    ir_top_err = math.fabs(ir_top_pid.reported_states[-1] - ir_top_pid.setpoint.data)
    ...
    if ir_bottom_err < 50:
        i += 1  
        return ir_bottom_pid.control_effort
    elif ir_top_pid.reported_states[-1] < 1000 and ir_bottom_pid.reported_states[-1] > 1000:
        i += 1
        return 1905
    else:
        return imu_wall_pid.control_effort
    return steering_cmd / i
...
def odroid():
    ...
    with pid_driver.Driver("IMU_CORNER",
        ...
        while not rospy.is_shutdown():
            ...
            steering_cmd = s_heuristic(ir_bottom_pid, ir_top_pid, imu_wall_pid, imu_corner_pid)
            try:
                steering_srv(CENTER + steering_cmd)
            except rospy.ServiceException, e:
